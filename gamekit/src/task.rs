...
use gamebuilder::manifest::StaticTaskDescriptor;
use log::{*};
...
impl TaskDispatcher {
    pub fn new(cycle_time_micros: u64) -> Self {
        ...
        let t_start = t_abs_start;
        let t_delta = std::time::Duration::from_micros(0u64);
        let t_frame_last = t_start;
        let t_frame_start = t_frame_last;
...
impl Task {
    ...
    fn thread_step(
...

        {
            ...
            if globals::options().show_statistics
                && dispatcher.statistics().is_updated() {
    ...
    fn thread_loop(
        ...
        while running_flag {
            ...
            if !running_ref.lock().unwrap().load(Ordering::Relaxed) {
                break;
            }
...
impl SyncTask {
    ...
    pub fn step(&mut self) {
        ...
        if globals::options().show_statistics
            && self.dispatcher.statistics().is_updated() {
    ...
    pub fn is_running(&self) -> bool {
        self.running && self.runnable.is_running()
    }
...
impl Tasks {
    ...
    pub fn get_lock(&self, id: &str) -> MutexGuard<'_, Task> {
        let material_ref = self.tasks.get(id).expect("material not found");
