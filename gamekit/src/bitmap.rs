...
use gamebuilder::manifest::StaticBitmapDescriptor;
...
impl Bitmap {
    ...
    fn from_image_obj(img: image::DynamicImage) -> Result<Self, Error> {
        ...
        let bitmap = Self::from_data(width, height, bits_per_pixel, pixels)?;
...
mod tests {
    ...
    #[test]
    fn test_bitmap() {
        let bmp = Bitmap::new(128,64, 32,0);
        assert_eq!(bmp.width(), 128);
        assert_eq!(bmp.height(), 64);
        assert_eq!(bmp.bits_per_pixel(), 32);
        assert_eq!(bmp.bytes_per_line(), 128*32/8);
        assert_eq!(bmp.size(), 128*64*32/8);
    }
}
