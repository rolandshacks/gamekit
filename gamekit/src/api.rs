...
/// Topology
pub type Topology = crate::material::Topology;
...
/// Point
pub type Point = crate::primitives::Point;

/// Frame
pub type Frame = crate::primitives::Frame;

/// Vertex
pub type Vertex = crate::primitives::Vertex;
...
/// Quad queue
pub type QuadQueue = crate::primitives::QuadQueue;

/// Tile (index)
pub type Tile = crate::primitives::Tile;

/// Tile queue
pub type TileQueue = crate::primitives::TileQueue;

/// Texture
pub type Texture = crate::texture::Texture;
...
/// Map
pub type Map = crate::maps::Map;

/// Shared map reference
pub type MapLockRef = crate::maps::MapLockRef;

/// Map level
pub type MapLevel = crate::maps::MapLevel;

/// Map layer
pub type MapLayer = crate::maps::MapLayer;

/// Input
pub type Input = crate::input::Input;

/// Manifest types (re-export)

pub type ApplicationDescriptorTable = gamebuilder::manifest::ApplicationDescriptorTable;
pub type StaticOptionsDescriptor = gamebuilder::manifest::StaticOptionsDescriptor;
pub type StaticDataDescriptor = gamebuilder::manifest::StaticDataDescriptor;
pub type StaticBitmapDescriptor = gamebuilder::manifest::StaticBitmapDescriptor;
pub type StaticTextureDescriptor = gamebuilder::manifest::StaticTextureDescriptor;
pub type StaticFontDescriptor = gamebuilder::manifest::StaticFontDescriptor;
pub type StaticShaderDescriptor = gamebuilder::manifest::StaticShaderDescriptor;
pub type StaticMaterialDescriptor = gamebuilder::manifest::StaticMaterialDescriptor;
pub type StaticTaskDescriptor = gamebuilder::manifest::StaticTaskDescriptor;
pub type StaticSampleDescriptor = gamebuilder::manifest::StaticSampleDescriptor;
pub type StaticMapDescriptor = gamebuilder::manifest::StaticMapDescriptor;
...
mod tests {
    ...
    impl Disposable for TestApp { fn dispose(&mut self) {} }

    #[test]
    fn test_api() {

        let mut app = TestApp::new().unwrap();
        let task_context = TaskContext::new("main", 0);
        app.on_init();
        app.on_ready();
        assert!(app.is_running());
        app.on_update();
        app.on_async_update(&task_context);
        app.on_metrics();
        app.on_draw();
        app.on_keystate_change(0x1, 0x0);
        app.on_shutdown();
    }
