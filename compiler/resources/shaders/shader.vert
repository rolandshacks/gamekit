...
void main() {
    ...
    float height = params.view_width; if (height <= 0.0) height = 1.0;

    float x = -1.0 + 2.0 * (push.offset_x + iPosition.x - params.x_min) / width;
    float y = -1.0 + 2.0 * (push.offset_y + iPosition.y - params.y_min) / height;
